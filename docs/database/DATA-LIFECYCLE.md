...
| البند       | التفاصيل                                                             |
| ----------- | -------------------------------------------------------------------- |
| **المعرف**  | `LIFECYCLE-001-PORTFOLIO-PLATFORM`                                   |
| **الإصدار** | `1.3.0`                                                              |
...
يحدد هذا المستند الآلات الحالية للحالة (State Machines)، والقيود الصارمة للانتقالات، ومسؤولي الانتقالات لجميع الكيانات الـ 16 في قاعدة البيانات (4 جداول Better Auth 1.7.2 للمصادقة والجلسات + 12 جدول بيانات للنظام).
...
- **السياسة الصارمة في PostgreSQL**:
  - **يُسمح فقط بـ**: `INSERT`
  - **يُحظر تماماً**: `UPDATE`, `DELETE`, `TRUNCATE` (مرفوضة بحرص عبر مشغلي DB في `drizzle/0001_audit-immutability.sql` على مستوى الصف للصفوف ومستوى العبارة للتفريغ).
